...
# «+» в имени файла ломает доставку на iPhone (в ссылках он читается
# как пробел), поэтому удаляется без замены: «Энергия+21» → «Энергия21».
FS_DROPPED = re.compile(r""\+"")
...
def safe_filename(title, maxlen=30):
    ...
    файла — он спотыкался на длинном имени. Кириллицу оставляем (короткое
    имя кириллицей читать привычнее), но делаем имя короче и без кавычек.

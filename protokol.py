...
# ---------- имя файла ----------
# Оставляем только буквы (любые), цифры, «_», «-», «+», «.», «(», «)» и пробел —
# всё остальное (в т.ч. «/» и типографские кавычки «» “” ‘’) заменяется пробелом.
...
def safe_filename(title, maxlen=70):
    ...
    в «ООО «ИНСТА» / АО «Энергия+21»». Мало того, даже когда «/» заменили
    на «-», типографские кавычки «» и общая длина названия иногда мешали
    открыть присланный файл на iPhone (при том что на Windows он открывался).
    Поэтому здесь: убираем повтор слова «протокол» в начале (оно и так есть
    в префиксе «Протокол_»), убираем все кавычки и любую пунктуацию, кроме
    «-», «+», «.», «()», пробелы меняем на «_» и обрезаем длину — короткое
    ASCII-совместимое имя надёжно открывается на любом устройстве."""
    s = unicodedata.normalize("NFC", title or "")
    s = LEAD_WORD_PROTOKOL.sub("", s)
    s = FS_ALLOWED.sub(" ", s)
